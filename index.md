...
This appears to work, for the most part, but as of early July 2024, the Firefly lab extensions will load, and show the landing page, but will not actually function correctly, in that they will not draw anything on the canvas.  I have asked for guidance in building a version with a source map so that I can trace the Javascript failures back to the actual source code that is failing to make the UI (client) connection to the Python (server) backend.  This is the same problem that means that with prebuilt extensions, which is how all Jupyterlab first-party extensions and most third-party extensions now behave, you will need a copy of the extension in each environment, and understanding how to force the UI to talk to the server flavor in the kernel it's running, rather than the environment it's loaded into, is somewhere between a lack of understanding available documentation on my part and a fundamental limitation of the prebuilt extension model.  I'm hoping that it's the first case, since surely we aren't the only people who want to run labextensions with server components where the UI and the running kernel are two different environments.

Otherwise, behaviorally it appears functionally identical in terms of what payload code it will run.  There is a change visible in the UI, in that I do not know how to hide the Python environment running Jupyterlab from the generated image.
